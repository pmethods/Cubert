...
void loop() {
  ...
  // True if no heartbeat comes from ANDROID for 10 seconds...  resets the bluetooth once a second.
  if( (millis() - btResetTime) > btReset)
  {
    btResetTime = millis();  //  buy another 5 seconds AND toss 5 in to give it a chance...
    digitalWrite(BT_POWER, LOW);
    delay(100);                     // enough to kill it?
    digitalWrite(BT_POWER, HIGH);
  }
...
void drivePWM(Door * door, int vector)
{
  ...
  if( vector != brake )
  {
    if( (analogRead(door->fbPin) >= (maxVal + door->maxCal)) && (vector > brake) )
    {
      vector = brake;
      if(debug) Serial.println("Command was forced to brake due to top limit");
    }
    else if( (analogRead(door->fbPin) <= (minVal + door->minCal)) && (vector < brake) )
    {
      vector = brake;
      if(debug) Serial.println("Command was forced to brake due to bottom limit");
    }
    else if( millis() > (door->lastTime + valInterval) )
    {
      if( (analogRead(door->fbPin) < (door->lastVal + hystVal)) && (analogRead(door->fbPin) > (door->lastVal - hystVal)) )
      {
        vector = brake;
        if(debug) Serial.println("Command was forced to brake due to stall detection");
      }
      else door->lastTime = millis();   // Timer resets once fired and movement detected outside hist
    }
  }
  else
  {
    // Brake only actions
    door->lastTime = millis();              // timer resets when we stop
  }

  // I believe at this point we have a lockup unless user releases to brake as the timer will not be reset once tripped.

  // Actions that take place for a brake OR run condition
  door->isRunning = ((vector == brake)? false : true);
  door->lastVal = analogRead(door->fbPin);             // grab the current position
  door->side.writeMicroseconds(vector);   // write the new vector (either speed or requested / forced brake)

  /*if(debug) Serial.print(door->lastTime);
  if(debug) Serial.print("  ");
  if(debug) Serial.println(door->lastVal);*/
